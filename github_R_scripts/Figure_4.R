...
ancom_results<-read.table("ancom_results_plant_bacteria_genus.txt", header = TRUE, sep = '\t',row.names = 1, quote = "", dec = ".")

heatmap_data<-feature_table %>%
  filter(ancom_results$detected_0.9 == "TRUE" , ancom_results$Abundance == "Y")
...
p4<- p3 + theme(axis.text = element_text(size = 8, face = "bold",
                ...
                axis.title = element_text(size = 16, face = "bold",
                                          color = "black"))+
  labs(x = "Bacterial Genus", y = "Linear Model Coefficient")
p4
bacteria_maslin<-p4
bacteria_maslin<-as.ggplot(bacteria_maslin)

pcombined<-(p5_mds_8plants+p5_mds_long)/(heatmap+bacteria_maslin)
